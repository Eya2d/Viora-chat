...
function closeCallWindowAsEnd() {
  const active = state.activeCall;
  if (!active) return closeCallWindowOnly();
  const action = active.direction === "incoming" && !active.localStream ? "reject" : "end";
  endLocalCall(true, action);
}
...
function playRemoteCallAudio(stream) {
  ...
  els.remoteCallAudio.srcObject = audioTracks.length ? new MediaStream(audioTracks) : stream;
...
function playRemoteCallVideo(stream) {
  ...
  els.remoteCallVideo.srcObject = videoTracks.length ? new MediaStream(videoTracks) : stream;
  els.remoteCallVideo.muted = true;
  els.remoteCallVideo.play?.().catch(() => {});
  els.callWindow.classList.add("has-remote-video");
  els.remoteCallAudio?.play?.().catch(() => {});
}
...
function isFloatingMenu(node) {
  return [els.overflowMenu, els.chatMenu, els.composerMenu, els.languagePanel].includes(node);
}

function hasVisibleFloatingMenu() {
  return [els.overflowMenu, els.chatMenu, els.composerMenu, els.languagePanel].some(isShown);
}

function hideMenuOverlayIfIdle() {
  if (!hasVisibleFloatingMenu()) hideFloatingElement(els.menuOverlay);
}
...
function hideFloatingElement(node) {
  ...
  if (isFloatingMenu(node)) hideMenuOverlayIfIdle();
}
...
function clearViewerDragHandlers() {
  els.viewerBody.onpointerdown = null;
  els.viewerBody.onpointermove = null;
  els.viewerBody.onpointerup = null;
  els.viewerBody.onpointercancel = null;
  els.viewerBody.onlostpointercapture = null;
  els.viewerBody.classList.remove("viewer-swipe-enabled", "viewer-dragging");
}

function bindViewerSwipeNavigation(onPrevious, onNext) {
  clearViewerDragHandlers();
  let startX = 0;
  let startY = 0;
  let lastX = 0;
  let lastY = 0;
  ...
  const resetDrag = () => {
    ...
    els.viewerBody.classList.remove("viewer-dragging");
  };

  els.viewerBody.classList.add("viewer-swipe-enabled");
  els.viewerBody.onpointerdown = (event) => {
    ...
    els.viewerBody.classList.add("viewer-dragging");
    els.viewerBody.setPointerCapture?.(event.pointerId);
  };
  els.viewerBody.onpointermove = (event) => {
    if (!isDragging || event.pointerId !== activePointerId) return;
    lastX = event.clientX;
    lastY = event.clientY;
  };
  els.viewerBody.onpointerup = (event) => {
    if (!isDragging || event.pointerId !== activePointerId) return;
    lastX = event.clientX;
    lastY = event.clientY;
  ...
  els.viewerBody.onpointercancel = resetDrag;
  els.viewerBody.onlostpointercapture = resetDrag;
}
...
window.vioraDownloadViewerFile = async (event) => {
  event?.preventDefault?.();
  const link = els.viewerOpenLink;
  const url = link?.dataset?.currentUrl || link?.href || "";
  const fileName = link?.dataset?.currentName || link?.getAttribute("download") || "attachment";
...
async function openAttachmentViewer(media, options = {}) {
  ...
  const setViewerDownloadTarget = (item) => {
    ...
    els.viewerOpenLink.dataset.currentName = item.name || "attachment";
  };
...
els.closeCallWindow?.addEventListener("click", closeCallWindowAsEnd);
