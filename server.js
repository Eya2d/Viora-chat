...
function normalizePushToken(token) {
  return String(token || "").trim().slice(0, 4096);
}
...
function removePushToken(token) {
  ...
  let changed = false;
  db.users.forEach((user) => {
    const before = Array.isArray(user.pushTokens) ? user.pushTokens.length : 0;
    user.pushTokens = (user.pushTokens || []).filter((item) => item.token !== normalized);
    if (user.pushTokens.length !== before) changed = true;
  });
  return changed;
}
...
function recipientUsersForMessage(message) {
  if (message.recipientId) {
    return db.users.filter((user) => user.id === message.recipientId);
  }
  return db.users.filter((user) => user.id !== message.userId);
}

async function sendPushForMessage(message) {
  const admin = initPushNotifications();
  if (!admin) {
    console.warn("Push skipped: FIREBASE_SERVICE_ACCOUNT_JSON or FIREBASE_SERVICE_ACCOUNT_BASE64 is not configured.");
    return;
  }
  const tokens = Array.from(new Set(recipientUsersForMessage(message)
    .flatMap((user) => Array.isArray(user.pushTokens) ? user.pushTokens.map((item) => item.token) : [])
    .filter(Boolean)));
  if (!tokens.length) {
    console.warn("Push skipped: recipient has no registered FCM token.", {
      recipientId: message.recipientId || "general",
      conversationId: message.conversationId || "general"
    });
    return;
  }
  const title = message.author || "Viora Chat";
  const body = messageNotificationText(message);
  const data = {
    title,
    body,
...
function getPushStatus(req, res) {
  const auth = requireAuth(req, res);
  if (!auth) return;
  json(res, 200, pushStatusForUser(auth.user));
}
...
async function sendPushTest(req, res) {
  ...
  if (!auth) return;
  const admin = initPushNotifications();
  if (!admin) return json(res, 503, { error: "Firebase Admin غير مفعل على السيرفر. أضف FIREBASE_SERVICE_ACCOUNT_JSON في Render ثم أعد النشر." });
  const tokens = Array.from(new Set((auth.user.pushTokens || []).map((item) => item.token).filter(Boolean)));
  if (!tokens.length) return json(res, 404, { error: "لا يوجد FCM token محفوظ لهذا الحساب. افتح التطبيق وثبّت APK الجديد وسجّل الدخول." });
  const result = await admin.messaging().sendEachForMulticast({
    tokens,
    notification: {
      title: "Viora Chat",
      body: "اختبار الإشعارات يعمل."
    },
    data: {
      title: "Viora Chat",
      body: "اختبار الإشعارات يعمل.",
...
async function createMessage(req, res) {
  ...
  sendPushForMessage(message).catch((error) => console.error("Push send failed:", error.message));
...
async function logout(req, res) {
  ...
  if (user) {
    ...
    if (ownsSession || ownsRememberToken) {
      ...
      if (pushToken) removePushToken(pushToken);
    }
  }
  saveDb();
